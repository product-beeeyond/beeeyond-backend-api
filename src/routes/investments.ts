...
// Buy property tokens
router.post('/buy', authenticate, requireKYC, validate(investmentSchema), async (req: AuthRequest, res) => {
...
router.post('/sell', authenticate, requireKYC, async (req: AuthRequest, res) => {
  const dbTransaction = await sequelize.transaction();
