...
export const createPropertySchema = Joi.object({
  ...
  propertyType: Joi.string()
    ...
    .messages({
      "any.only":
        "Property type must be one of: residential, commercial, mixed_use, industrial",
...
export const multisigWalletSchema = Joi.object({
  ...
  propertyManagerPublicKey: Joi.string()
    .pattern(stellarPublicKeyPattern)
    .required()
    .messages({
      "string.pattern.base": "Invalid Stellar public key format",
    }),
  adminPublicKeys: Joi.array()
    .items(
      Joi.string().pattern(stellarPublicKeyPattern).messages({
        "string.pattern.base": "Invalid Stellar public key format",
      })
    )
    .min(2)
...
export const multisigTransactionSchema = Joi.object({
  ...
  category: Joi.string()
    .valid("fund_management", "governance", "revenue_distribution", "emergency")
    .required(),
...
export const revenueDistributionSchema = Joi.object({
  ...
  revenueSource: Joi.string()
    .valid("rental", "sale", "appreciation", "other")
    .required(),
...
export const validateProposalData = (
  proposalType: string,
  proposalData: any
) => {
...
export const validateDynamicProposalData = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const kycApprovalSchema = Joi.object({
  ...
  reason: Joi.when("action", {
    ...
    otherwise: Joi.string().min(10).max(500).optional().messages({
...
export const bulkKycActionSchema = Joi.object({
  ...
  reason: Joi.when("action", {
    ...
    then: Joi.string().min(10).max(500).required().messages({
      ...
      "string.max": "Rejection reason cannot exceed 500 characters",
      "any.required":
        "Rejection reason is required when rejecting KYC applications",
    }),
    otherwise: Joi.string().min(10).max(500).optional().messages({
      "string.min": "Reason must be at least 10 characters",
      "string.max": "Reason cannot exceed 500 characters",
...
export const kycOverviewQuerySchema = Joi.object({
  ...
  status: Joi.string()
    ...
    .messages({
      "any.only":
        "Status must be one of: pending, under_review, verified, rejected",
    }),
 ...
 */
export const validateBulkKYCAction = [validateRequest(bulkKycActionSchema)];

/**
 * Validation middleware for pending KYC query parameters
 */
export const validatePendingKYCQuery = [validateQuery(pendingKycQuerySchema)];

/**
 * Validation middleware for KYC overview query parameters
 */
export const validateKYCOverviewQuery = [validateQuery(kycOverviewQuerySchema)];

/**
 * Validation middleware for KYC details endpoint
 */
export const validateKYCDetails = [validateParams(userIdParamSchema)];
