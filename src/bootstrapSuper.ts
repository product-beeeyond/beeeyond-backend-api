 ...
 */
const validateSuperAdminEnv = (): { email: string; password: string; firstName: string; lastName: string } => {
...
export async function ensureSuperAdmin(): Promise<void> {
  try {
    ...
    logger.info('❌ No super admin found. Creating from environment variables...');
    ...
    if (existingUser) {
      logger.warn('⚠️ User with super admin email already exists but with different role:', {
        email,
